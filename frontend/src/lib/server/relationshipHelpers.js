 ...
 * @param {string} type - Relationship type ("mother", "father", "spouse")
 * @returns {Object} Normalized relationship { person1Id, person2Id, type, parentRole }
 */
export function normalizeRelationship(person1Id, person2Id, type) {
 ...
 * Only "mother", "father", and "spouse" are valid
 *
 * @param {string} type - Relationship type
 * @returns {Object} Validation result { valid: boolean, error: string|null }
 */
export function validateRelationshipType(type) {
  const validTypes = ['mother', 'father', 'spouse']
  ...
  if (!validTypes.includes(type)) {
    return {
      valid: false,
      error: 'Invalid relationship type. Must be: mother, father, or spouse'
    }
  }
